...
export class AES {
    ...
    constructor(texto: string, llave: string, encriptar: boolean) {
        //texto no debe usar el caracter alfabeto[171]
        this.bloques = new Array<any>();
        this.llave = new Array<any>();
        while (texto.length % 16 != 0 && encriptar)
            texto += " ";
        while (llave.length % 16 != 0)
            llave += " ";
        this.convertir(llave, 0, true);
        if (encriptar)
            this.Encripatar(texto);
        else
            this.Desencriptar(texto);
        this.imprimirTexto();
    }
    public Encripatar(texto: string) {
        this.generarSubLlaves();
        for (let i = 0; i < texto.length; i += 16) {
            this.convertir(texto, i);
            if (i == 0)
                this.Encripatacion(this.bloques[i / 16], constanteIV);
            else
                this.Encripatacion(this.bloques[i / 16], this.bloques[(i / 16) - 1]);
            //this.imprimir(this.bloques[i / 16]);
        }
    }
    public Desencriptar(texto: string) {
        this.generarSubLlaves();
        let temporal: Array<Array<number>> = constanteIV;
        ...
        for (let k = 0; k < this.bloques.length; ++k) {
            for (let i = 0; i < this.bloques[k].length; ++i) {
                for (let j = 0; j < this.bloques[k][i].length; ++j) {
                    if (this.bloques[k][i][j] >= 171) {
                        texto += alfabeto[171];
                        this.bloques[k][i][j] -= 171;
                    }
                    texto += alfabeto[this.bloques[k][i][j]];
    ...
    private convertir(frase: string, posI: number, llave: boolean = false) {
        ...
        for (let i = 0; i < 4; ++i) {
            for (let j = 0; j < 4; ++j) {
                if (frase[posI + 4 * i + j + add] == alfabeto[171]) {
                    temp[i][j] = 171;
                    add++;
                }
                else
                    temp[i][j] = 0;
                for (let k = 0; k < 171; k++) {
                    if (frase[posI + 4 * i + j + add] == alfabeto[k])
                        temp[i][j] += k;
                }
            }
        }
